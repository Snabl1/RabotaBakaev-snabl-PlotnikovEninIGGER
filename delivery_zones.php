...
// Получение всех зон доставки (и активных, и неактивных) для отображения списка
$zones = $pdo_delivery->query("SELECT * FROM delivery_zones ORDER BY zone_id")->fetchAll();

// Для формы редактирования — загружаем зону один раз по ID
$zone = null;
if (isset($_GET['edit'])) {
    $edit_zone_id = (int) $_GET['edit'];
    foreach ($zones as $z) {
        if ((int)$z['zone_id'] === $edit_zone_id) {
            $zone = $z;
            break;
        }
    }
    if (!$zone) {
        $_SESSION['flash'] = ['type' => 'error', 'message' => 'Зона не найдена.'];
                ...
                <div class="form-group">
                    <label>
                        <input type="checkbox" name="is_active" value="1"
                               <?= (isset($zone) && $zone['is_active']) ? 'checked' : '' ?>>
                        Активна
                    </label>
                </div>
