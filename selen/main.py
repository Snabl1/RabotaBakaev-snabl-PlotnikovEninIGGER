...
def run_code_standards_check():
    ...
    quality = {
        "php_syntax_checked": checked,
        "php_syntax_errors": [{"file": rel, "message": (msg or "").strip()[:500]} for rel, msg in errors],
        "php_syntax_ok": len(errors) == 0,
        "psr_tool": None,
        "psr_standard": "PSR2",
        "psr_ok": None,
        "psr_output_tail": None,
    }
    if errors:
        for rel, msg in errors:
            console.print(f"[red]✗ {rel}:[/red] {msg.strip()[:200]}")
        console.print(f"[red]Всего ошибок: {len(errors)} из {checked} файлов[/red]")
        _report_code_quality = quality
        return False
    ...
    try:
        ...
        quality["psr_tool"] = "phpcs"
        if r.returncode == 0:
            console.print("[green]✓ Соответствие PSR-1/PSR-2 (phpcs): нарушений не найдено[/green]")
            quality["psr_ok"] = True
            quality["psr_output_tail"] = ""
        else:
            out = (r.stdout or "") + (r.stderr or "")
            console.print("[yellow]⚠ PSR-1/PSR-2 (phpcs):[/yellow]")
            for line in out.strip().split("\n")[-15:]:
                console.print(f"[dim]{line}[/dim]")
            quality["psr_ok"] = False
            quality["psr_output_tail"] = "\n".join(out.strip().split("\n")[-50:])
    except FileNotFoundError:
        console.print("[dim]phpcs не установлен — для проверки PSR-1/PSR-2: composer require squizlabs/php_codesniffer[/dim]")
        quality["psr_tool"] = None
    except Exception as e:
        console.print(f"[dim]phpcs: {e}[/dim]")
        quality["psr_tool"] = "phpcs_error"
        quality["psr_output_tail"] = str(e)
    _report_code_quality = quality
    return True


def run_code_complexity_check():
    """Метрика сложности (по t2.txt): считаем символы/операторы/операнды/циклы/условия/пробелы/энтеры."""
    console.print("\n[bold cyan]🧮 Проверка сложности кода (метрика по счётчикам)...[/bold cyan]")
    root = cfg.PROJECT_ROOT
    files = []
    for dirpath, dirnames, filenames in os.walk(root):
        # исключаем тяжёлые/нерелевантные папки
        if any(x in dirpath for x in ("\\backup", "/backup", "\\vendor", "/vendor", "\\node_modules", "/node_modules", "\\.git", "/.git", "\\selen", "/selen")):
            continue
        for f in filenames:
            if f.endswith(".php"):
    ...
    # Операторы — сначала длинные, чтобы не считать дважды
    import re
    op_re = re.compile(r"(===|!==|==|!=|<=|>=|=>|->|::|\+\+|--|\+=|-=|\*=|/=|%=|\.=|&&|\|\||[+\-*/%=&|^~!.?:<>])")
    loops_re = re.compile(r"\b(foreach|for|while|do)\b", re.IGNORECASE)
    cond_re = re.compile(r"\b(if|elseif|else|switch|case|catch)\b", re.IGNORECASE)
    ident_re = re.compile(r"\b[A-Za-z_][A-Za-z0-9_]*\b")
    num_re = re.compile(r"\b\d+(?:\.\d+)?\b")
    str_re = re.compile(r"(?:'[^'\\]*(?:\\.[^'\\]*)*'|\"[^\"\\]*(?:\\.[^\"\\]*)*\")", re.DOTALL)

    php_keywords = set("""
        if else elseif switch case break continue return try catch finally throw
        for foreach while do function class interface trait extends implements public private protected static abstract final
        new clone echo print include require include_once require_once namespace use global isset empty unset
    ...
    for path in files:
        ...
        newlines = text.count("\n")
        tabs = text.count("\t")
        operators = len(op_re.findall(text))
        loops = len(loops_re.findall(text))
        conditions = len(cond_re.findall(text)) + text.count("?")  # тернарный оператор как условие

        # Операнды — приближённо: идентификаторы (без ключевых слов) + числа + строковые литералы
        idents = [w for w in ident_re.findall(text) if w.lower() not in php_keywords]
        operands = len(idents) + len(num_re.findall(text)) + len(str_re.findall(text))

        totals["chars"] += chars
        totals["spaces"] += spaces
        totals["newlines"] += newlines
        totals["tabs"] += tabs
        totals["operators"] += operators
        totals["loops"] += loops
        totals["conditions"] += conditions
    ...
    per_file.sort(key=lambda x: (x["conditions"] + x["loops"] + x["operators"]), reverse=True)
    data = {"totals": totals, "top_files": per_file[:10]}
    global _report_code_complexity
    _report_code_complexity = data
    console.print(f"[green]✓ Файлов: {totals['files']} | символов: {totals['chars']} | операторов: {totals['operators']} | операндов: {totals['operands']}[/green]")
    return True
...
def git_autosave(root: str, message: str):
    """Автосейв кода в git (по t2.txt). Без правки git config — автор/коммитер через env."""
    info = {"enabled": True, "ok": False, "message": message}
    if not _git_available():
        info["enabled"] = False
        info["error"] = "git not available"
        return info
    if not _git_ensure_repo(root):
        info["error"] = "cannot init git repo"
        return info
    env = os.environ.copy()
    env.setdefault("GIT_AUTHOR_NAME", "AutoSave Bot")
    env.setdefault("GIT_AUTHOR_EMAIL", "autosave@local")
    env.setdefault("GIT_COMMITTER_NAME", "AutoSave Bot")
    env.setdefault("GIT_COMMITTER_EMAIL", "autosave@local")
    try:
        subprocess.run(["git", "add", "-A"], cwd=root, env=env, capture_output=True, text=True, timeout=30)
        st = subprocess.run(["git", "status", "--porcelain"], cwd=root, env=env, capture_output=True, text=True, timeout=10)
        if not st.stdout.strip():
            info["ok"] = True
            info["skipped"] = "no changes"
            return info
        r = subprocess.run(["git", "commit", "-m", message], cwd=root, env=env, capture_output=True, text=True, timeout=60)
        info["ok"] = (r.returncode == 0)
        if not info["ok"]:
            info["error"] = (r.stderr or r.stdout or "").strip()[:500]
        return info
    except Exception as e:
        info["error"] = str(e)
        return info
...
def run_backup():
    """Бекап важных файлов проекта в backup/. Возвращает путь к бэкапу или None."""
    ...
    global _report_backup_info
    if _report_backup_info is None:
        _report_backup_info = []
    _report_backup_info.append({"path": target, "created_at": datetime.now().isoformat()})
    return target


def try_fix_psr_with_phpcbf(root: str):
    """Опционально (t2.txt): меняем код авто-фиксом, чтобы был смысл в 'до/после'."""
    if not _env_flag("AUTO_FIX_PSR", False):
        return {"enabled": False}
    console.print("\n[bold cyan]🛠 Авто-фикс PSR-2 (phpcbf, если установлен)...[/bold cyan]")
    info = {"enabled": True, "tool": None, "ok": False, "output_tail": None}
    try:
        subprocess.run(["phpcbf", "-V"], capture_output=True, text=True, timeout=5, cwd=root)
        info["tool"] = "phpcbf"
    except FileNotFoundError:
        console.print("[yellow]⚠ phpcbf не найден (часть PHP_CodeSniffer). Авто-фикс пропущен.[/yellow]")
        info["tool"] = None
        return info
    except Exception as e:
        info["tool"] = "phpcbf_error"
        info["output_tail"] = str(e)
        return info
    try:
        r = subprocess.run(
            ["phpcbf", "--standard=PSR2", "--extensions=php", "--ignore=selen,vendor,node_modules", root],
            capture_output=True,
            text=True,
            timeout=300,
            cwd=root,
        ...
        info["output_tail"] = "\n".join(out.strip().split("\n")[-50:])
        if info["ok"]:
            console.print("[green]✓ Авто-фикс выполнен[/green]")
        else:
            console.print("[yellow]⚠ Авто-фикс завершился с предупреждениями/ошибками[/yellow]")
        return info
    except Exception as e:
        info["output_tail"] = str(e)
        return info
...
class PHPSiteTester:
    ...
    def record_result_criterion(self, criterion, test_name, success, details="", screenshot_info=None):
        """Результат с критерием (валидность/верификация/свой)."""
        status = "✅ УСПЕХ" if success else "❌ ОШИБКА"
        color = "green" if success else "red"
        det = details + (f" (скриншот: {screenshot_info['name']})" if screenshot_info else "")
        self.results.append({
        ...
        self.console.print(f"[{color}]✓ {criterion} · {test_name}: {details}[/{color}]" if success else f"[{color}]✗ {criterion} · {test_name}: {details}[/{color}]")
    ...
    def test_login(self):
        ...
        try:
            ...
            if "Выход" in self.driver.page_source or "index.php" in self.driver.current_url:
                self.record_result_criterion("Верификация", "Логин и пароль", True, "Вход выполнен", scr)
                return True
            if "Неверный" in self.driver.page_source or "ошибка" in self.driver.page_source.lower():
                self.record_result_criterion("Верификация", "Логин и пароль", False, "Неверный логин/пароль", scr)
                return False
            self.record_result_criterion("Верификация", "Логин и пароль", True, "Переход после входа", scr)
            return True
        except Exception as e:
            scr = self.take_screenshot("Логин_ошибка", str(e))
            self.record_result_criterion("Верификация", "Логин и пароль", False, str(e), scr)
            return False

    def test_invalid_login_validation(self):
        """Валидность: неверные креды должны давать ошибку."""
        self.console.print("\n[bold cyan]🧪 Валидность: неверный логин...[/bold cyan]")
        try:
            self.driver.get(f"{self.base_url}/login.php")
            time.sleep(1)
            self.safe_send_keys("input[name='username']", "not_a_user_" + str(random.randint(1000, 9999)))
            self.safe_send_keys("input[name='password']", "wrongpass")
            btn = self.wait_for_element("button[type='submit']") or self.wait_for_element("input[type='submit']")
            if btn:
                btn.click()
            time.sleep(1.5)
            scr = self.take_screenshot("Неверный_логин", "Проверка сообщения об ошибке")
            ok = ("неверн" in self.driver.page_source.lower()) or ("ошиб" in self.driver.page_source.lower())
            self.record_result_criterion("Валидность", "Неверный логин/пароль", ok, "Ожидаем сообщение об ошибке", scr)
            return ok
        except Exception as e:
            self.record_result_criterion("Валидность", "Неверный логин/пароль", False, str(e), self.take_screenshot("Неверный_логин_ошибка", str(e)))
            return False

    def test_access_control_custom(self):
        """Свой критерий: безопасность — без сессии админ-страницы должны редиректить на логин."""
        self.console.print("\n[bold cyan]🛡️ Свой критерий: контроль доступа...[/bold cyan]")
        try:
            # новый драйвер без куков (чтобы не ломать основной поток)
            options = webdriver.ChromeOptions()
            options.add_argument("--no-sandbox")
            options.add_argument("--disable-dev-shm-usage")
            options.add_argument("--disable-gpu")
            ...
            d.implicitly_wait(5)
            admin_pages = ["orders.php", "clients.php", "components.php", "categories.php", "delivery_zones.php", "warranties.php", "report.php", "backups.php"]
            ok_all = True
            for p in admin_pages:
                d.get(f"{self.base_url}/{p}")
                time.sleep(1)
                url = d.current_url.lower()
                ok = ("login.php" in url) or ("вход" in (d.title or "").lower()) or ("login" in url)
                ok_all = ok_all and ok
            self.record_result_criterion("Свой", "Контроль доступа (без сессии)", ok_all, "Админ-страницы должны требовать вход", None)
            d.quit()
            return ok_all
        except Exception as e:
            self.record_result_criterion("Свой", "Контроль доступа (без сессии)", False, str(e))
            return False
        ...
        for name, url in menu_items:
            try:
                ...
                if name.lower() in self.driver.title.lower() or name[:4].lower() in self.driver.find_element(By.TAG_NAME, "body").text.lower():
                    self.record_result_criterion("Верификация", f"Навигация: {name}", True, self.driver.title, scr)
                else:
                    self.record_result_criterion("Верификация", f"Навигация: {name}", False, "Контент не найден", scr)
            except Exception as e:
                scr = self.take_screenshot(f"Навигация_{name}_ERROR", str(e))
                self.record_result_criterion("Верификация", f"Навигация: {name}", False, str(e), scr)
        ...
        try:
            ...
            if add_btn:
                ...
                if "успешно" in self.driver.page_source.lower():
                    self.record_result_criterion("Валидность", "Добавление клиента", True, "Клиент добавлен", scr2)
                else:
                    self.record_result_criterion("Валидность", "Добавление клиента", False, "Нет сообщения об успехе", scr2)
            else:
                self.record_result_criterion("Валидность", "Добавление клиента", False, "Кнопка добавления не найдена", scr)
        except Exception as e:
            self.record_result_criterion("Валидность", "Добавление клиента", False, str(e), self.take_screenshot("Клиенты_ошибка", str(e)))
    ...
    def test_orders(self):
        ...
        if form and grand:
            self.record_result_criterion("Верификация", "Форма заказа", True, "Форма и итог найдены", scr)
        else:
            self.record_result_criterion("Верификация", "Форма заказа", False, "Форма или #grandTotal не найдены", scr)
    ...
    def test_categories(self):
        ...
        if inp:
            ...
            if btn:
                ...
                self.record_result_criterion("Валидность", "Добавление категории", "успешно" in self.driver.page_source.lower(), "Проверка по тексту", scr)
        else:
            self.record_result_criterion("Верификация", "Категории", False, "Поле category_name не найдено")
    ...
    def test_components(self):
        ...
        self.record_result_criterion("Верификация", "Комплектующие", tbl is not None, "Таблица найдена" if tbl else "Таблица не найдена", scr)
    ...
    def test_delivery_zones(self):
        ...
        self.record_result_criterion("Верификация", "Зоны доставки", z is not None, "Форма найдена" if z else "Форма не найдена", scr)
    ...
    def test_warranties(self):
        ...
        self.record_result_criterion("Верификация", "Гарантии", "гарант" in self.driver.page_source.lower() or "warranty" in self.driver.page_source.lower(), "Страница загружена", scr)

    def run_all_tests(self):
        self.start_time = time.time()

        # 0. Сложность кода (t2.txt)
        ...
        # Автосейв в git (схема выбирается env GIT_SAVE_SCHEME)
        git_scheme = (os.environ.get("GIT_SAVE_SCHEME") or "per_run").strip().lower()
        do_git = _env_flag("AUTO_GIT", True)
        global _report_git_info
        _report_git_info = {"enabled": do_git, "scheme": git_scheme, "runs": []}
        if do_git and git_scheme in ("before_after_fix", "per_run"):
            _report_git_info["runs"].append(git_autosave(root, f"Auto-save (before tests) {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"))

        # 2. Стандарты кода
        run_code_standards_check()

        # Опционально: авто-фикс PSR (это и есть "изменение кода" для задания)
        fix_info = try_fix_psr_with_phpcbf(root)
        if fix_info.get("enabled"):
            # "после изменения" — бэкап и/или git
            if getattr(cfg, "DO_BACKUP", False):
                run_backup()
            if do_git and git_scheme == "before_after_fix":
                _report_git_info["runs"].append(git_autosave(root, f"Auto-save (after PSR fix) {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"))
            # перезапускаем проверку, чтобы в отчёте было актуально
            run_code_standards_check()
        ...
        try:
            ...
            # Включаем/выключаем критерии через env TEST_CRITERIA (по умолчанию — все)
            criteria = set(_env_csv("TEST_CRITERIA", "validity,verification,custom"))
            selected = []
            if "verification" in criteria:
                selected.extend([
            ...
            if "validity" in criteria:
                # негативный и позитивные сценарии ввода
                self.test_invalid_login_validation()
                selected.extend([
    ...
    def write_web_report(self):
        ...
        try:
            ...
            data = {
                ...
                "code_quality": _report_code_quality,
